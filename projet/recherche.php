...
<body>
    ...
    ?>
    <?php
        $recherche = $bd->query('SELECT nom_art from artisan');
            if(isset($_GET['barre']) && !empty($_GET['barre'])){
                $barre = htmlspecialchars($_GET['barre']);
                $recherche = $bd->query("SELECT * from artisan where nom_art like '%$barre%' or metier like '%$barre%' or wilaya_art like '%$barre%'");
            }
    ?>    
        ...
        <?php while($r = $recherche->fetch()){?>
            <div class="carte">
                <div class="bleu1">
                    ...
                    <a href="profil_artisan.php?id=<?php echo $r['id_art'] ?>" style="text-decoration: none; color: #5491f6; margin: 10px;"><b class="nom"><?php echo $r['nom_art'] ."<br>". $r['prenom_art'] ?></a></b>
                    <button class="z"><a href="rdv.php?id=<?php echo $r['id_art'] ?>" style="text-decoration: none; color: #5491f6;"><h3>Prendre rendez-vous</h3></a></button>
                </div>
